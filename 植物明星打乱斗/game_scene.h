...
public:
	...
	void on_draw(const Camera& camera) {
		...
		if (is_debug) {
			...
			outtextxy(15, 15, _T("开启调试模式，按“F1”关闭"));
		}
	...
	void on_exit() {
		...
		bullet_list.clear();//没有
		main_camera.reset();
	}

private:
	POINT pos_img_sky = { 0 };				// 天空背景图片
	POINT pos_img_hill = { 0 };				// 山脉背景图片
	...
	POINT pos_img_winner_bar = { 0 };		// 结算动画背景位置
	POINT pos_img_winner_text = { 0 };		// 结算动画文本位置
	int pos_x_img_winner_bar_dst = 0;		// 结算动画背景移动的目标位置
	int pos_x_img_winner_text_dst = 0;		// 结算动画文本移动的目标位置
	Timer timer_winnner_slide_in;			// 结算动画滑入定时器
	Timer timer_winnner_slide_out;			// 结算动画滑出定时器
	bool is_slide_out_started = false;		// 结算动画是否开始滑出
	const float speed_winnner_bar = 3.0f;	// 结算动画背景移动速度
	const float speed_winnner_text = 1.5f;	// 结算动画文字移动速度
};