...
private:
	...
	void on_draw(const Camera& camera) {
		...
		if (is_sun_text_visible) {
			...
			text_pos.x = pos.x - (size.x - frame->getwidth()) / 2;
			text_pos.y = pos.y - frame->getheight();
			animation_sun_text.on_drow(camera, (int)text_pos.x, (int)pos.y);
		}
	}
	...
	void on_attack_ex() {
		...
		Vector2 bullet_pos, bullet_velocity;
		const Vector2& bullet_size = bullet->get_size();
		const Vector2& target_size = target_player->get_size();
		const Vector2& target_pos = target_player->get_pos();
...
private:
	...
	Animation animation_sun_text;					// 特殊攻击头部文本动画
